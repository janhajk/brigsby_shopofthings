...
if ( hoot_page_header_attop() ) {
	get_template_part( 'template-parts/loop-meta-no-headers' ); // Loads the template-parts/loop-meta.php template to display Title Area with Meta Info (of the loop)
...
<div class="hgrid main-content-grid">
	...
	<main <?php hybridextend_attr( 'content' ); ?>>
		...
		if ( have_posts() ) :
			...
			if ( ! hoot_page_header_attop() ) {
				hoot_display_loop_title_content( 'post', 'page.php' );
				get_template_part( 'template-parts/loop-meta-no-headers' ); // Loads the template-parts/loop-meta-no-headers.php template to display Title Area with Meta Info (of the loop)
			}
			?>
...
<style>
    h1 { font-size: 2.5em; } /* Sie können die Größe nach Bedarf anpassen */
</style>
