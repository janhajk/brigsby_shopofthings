...
function display_icon_row($title, $taxonomy, $attribute, $alt_text) {
    ...
    foreach ($terms as $term_name) {
        ...
        if ($term) {
            ...
            if ($thumbnail_url) {
                $thumbnail_elements[$term_name] = '<a href="' . esc_url($term_link) . '" class="icon-link"><img src="' . esc_url($thumbnail_url) . '" alt="' . $alt_text . '" class="icon-image" title="' . esc_attr($term_name) . '" height="30"></a>';
            } else {
